...
export default function AddAppointmentModal({
  ...
  const goBack = () => {
    setStep((s) =>
      s === "client" ? "services" : s === "time" ? "client" : "time"
    );
  };

  const goNext = () => {
    setStep((s) =>
      s === "services" ? "client" : s === "client" ? "time" : "summary"
    );
  };
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 grid grid-cols-1 md:grid-cols-[1fr_320px] gap-4">
          <div className="min-h-[240px]">
            ...
            <ClientStep
              ...
              onChange={(c: ClientMini | null) =>
                setDraft((p) => ({ ...p, client: c }))
              }
            />
            ...
            {step === "summary" && (
              <div>
                <SummaryPanel
                  ...
                  client={draft.client}
                  dateISO={
                    draft.date ? draft.date.toISOString().slice(0, 10) : null
                  }
                  start={draft.start}
                />
              </div>
        ...
        >
          <div className="text-xs text-gray-500">
            Changes won’t be saved to the database until you press{" "}
            <b>Confirm</b>.
          </div>
            ...
            {canGoNext ? (
              ...
              <button
                onClick={async () => {
                  ...
                  const durationMin = draft.services.reduce(
                    (a, s) => a + (s.duration || 0),
                    0
                  );
                  const endMs = startMs + durationMin * 60 * 1000;
                  const priceTotal = draft.services.reduce(
                    (a, s) => a + (s.price || 0),
                    0
                  );
